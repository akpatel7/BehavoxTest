...
export class AppComponent {
  ...
  customComparator: (a: string, b: string) => number = this.customNameComparator;
  
  constructor(private orderPipe: OrderPipe) {
    this.sortedCollection = orderPipe.transform(this.emails, 'name');
  ...
  customNameComparator(itemA: string, itemB: string): number {
    return itemA > itemB ? 1 : -1;
  }
  
  customDateComparator(itemA: string, itemB: string): number {
    return itemA > itemB ? 1 : -1;
  }
